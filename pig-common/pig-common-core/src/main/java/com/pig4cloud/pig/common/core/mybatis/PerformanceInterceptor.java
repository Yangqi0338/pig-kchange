...
import static cn.hutool.db.sql.SqlFormatter.format;
...
public class PerformanceInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (isProxyClass(firstArg.getClass())) {
            statement = (Statement) forObject(firstArg).getValue("h.statement");
        } else {
            statement = (Statement) firstArg;
        }
        MetaObject stmtMetaObj = forObject(statement);
        try {
        ...
        MetaObject metaObject = forObject(target);
    ...
    private int indexOfSqlStart(String sql) {
        ...
        Set<Integer> set = Stream.of(upperCaseSql.indexOf("SELECT "), upperCaseSql.indexOf("UPDATE "),
                upperCaseSql.indexOf("INSERT "), upperCaseSql.indexOf("DELETE ")).collect(toSet());
        ...
        List<Integer> list = set.stream().collect(toList());
     ...
     * @return sql语句
     */
    private String sqlFormat(String boundSql, boolean format) {
        if (format) {
            return format(boundSql);
        }
        return boundSql;
    }
